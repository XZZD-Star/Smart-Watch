...
我会按 `karpathy-guidelines` 做最小重构：先只动实时链路，不碰 OneNet 协议和 AI 模型。因为你的项目规则要求删除/替换代码前先确认，我会先把要改的具体范围核清楚，再确认一次会移除哪些 ISR 内业务逻辑。
...
我可以开始做第一阶段优化，但按你项目规则，这次会涉及“替换 ISR 内原有业务处理逻辑”，需要你先确认一次。

我准备只做这些最小改动：

1. `USART1_IRQHandler` / `USART3_IRQHandler`  
   把中断里的 `process_pose_packet(...)` 和大块 `memset(...)` 移出 ISR，改成“复制原始帧 + 置位待处理标志”。

2. `Task1`  
   在任务循环里处理待解析的上臂/前臂原始帧，再继续使用现有融合、AI、规则、跌倒逻辑。

3. 任务优先级  
   `Task1` 从 `osPriorityLow` 提高到 `osPriorityAboveNormal`；`Task2` 从 `osPriorityNormal` 降到 `osPriorityBelowNormal` 或 `Low`，让动作识别优先于网络。

4. NVIC 优先级  
   传感器 UART/DMA 保持 `5`；ESP8266、UART4、UART7、TIM2 适当后移到 `6~8`，不碰 FreeRTOS 的 `configMAX_SYSCALL_INTERRUPT_PRIORITY = 5`。

5. 不动的部分  
   不重写 OneNet/MQTT，不动 AI 模型，不拆出新任务，不大改 BSP。

请确认：**允许我替换上述 ISR 内处理逻辑并调整任务/NVIC 优先级吗？**
...
我会按确认范围直接改第一阶段：不拆新任务，只把两路传感器解析从 ISR 挪到 `Task1`，并调整优先级。UART4 命令这次先不动，避免把改动面扩大。
...
我发现有一个关键细节：时间对齐不能在任务里重新取时间，否则两路帧的时间会被任务调度“压近”。所以我会在 ISR 里只保留轻量的 DWT 时间戳，然后把解析和融合放到 `Task1`。
...
我现在开始改文件。改动会集中在 [Core/Src/stm32h7xx_it.c](<D:/Qian Start/Smart Watch/H743/Core/Src/stm32h7xx_it.c>)、[Core/Src/freertos.c](<D:/Qian Start/Smart Watch/H743/Core/Src/freertos.c>)、[Core/Src/usart.c](<D:/Qian Start/Smart Watch/H743/Core/Src/usart.c>)、[Core/Src/dma.c](<D:/Qian Start/Smart Watch/H743/Core/Src/dma.c>) 和 [Core/Src/tim.c](<D:/Qian Start/Smart Watch/H743/Core/Src/tim.c>)。
...
第一轮代码已经完成：中断里不再做传感器字符串解析和融合，改为缓存原始帧并唤醒 `Task1`。我现在做静态核查，重点看函数签名、外部符号、优先级和是否还有 ISR 内重处理残留。
...
静态 diff 看起来符合预期。接下来我会检查是否有明显语法/空白问题，并确认 `Task1` 唤醒路径、优先级分层和中断内重处理都已经按第一阶段目标收敛。
...
已完成第一阶段优化，范围保持很小，没有动 OneNet 协议、AI 模型和 BSP 大结构。

主要改动：

- [stm32h7xx_it.c](<D:/Qian Start/Smart Watch/H743/Core/Src/stm32h7xx_it.c:396>)：`USART1/USART3` 中断里不再解析姿态字符串、不再融合，只缓存原始帧并记录 ISR 时间戳。
- [stm32h7xx_it.c](<D:/Qian Start/Smart Watch/H743/Core/Src/stm32h7xx_it.c:876>)：新增轻量缓存和 `Task1` 通知逻辑。
- [stm32h7xx_it.c](<D:/Qian Start/Smart Watch/H743/Core/Src/stm32h7xx_it.c:973>)：新增 `Motion_ProcessPendingPosePackets()`，把原来的解析/融合放回任务上下文执行。
- [freertos.c](<D:/Qian Start/Smart Watch/H743/Core/Src/freertos.c:127>)：`Task1` 提升到 `osPriorityAboveNormal`，`Task2` 降到 `osPriorityBelowNormal`。
- [freertos.c](<D:/Qian Start/Smart Watch/H743/Core/Src/freertos.c:346>)：`Task1` 改为优先处理待解析传感器包，并用任务通知替代原来的固定 `osDelay(10)` 轮询等待。
- [dma.c](<D:/Qian Start/Smart Watch/H743/Core/Src/dma.c:47>)、[usart.c](<D:/Qian Start/Smart Watch/H743/Core/Src/usart.c:370>)、[tim.c](<D:/Qian Start/Smart Watch/H743/Core/Src/tim.c:172>)：做了 NVIC 分层，传感器保持 `5`，UART4 为 `6`，ESP8266 为 `7`，屏幕/USART2/TIM2 为 `8`。

验证情况：

- `git diff --check` 通过，没有空白格式错误。
- 本机 PATH 里没找到 `armclang`、`UV4`、`fromelf`，所以我没法直接跑 Keil 编译。你需要用 Keil 编译一次确认。

当前还保留的已知点：`UART4` 的 `start/clear` 命令仍在中断里直接处理，`USART6` 的 ESP8266 字节喂入也还在回调里。第一阶段先不扩大改动面；下一阶段可以专门把这两个路径也任务化。
