...
我把你上一轮确认理解为：可以清理我列出的候选项。接下来只处理这三类：备份目录、`stm32h7xx_it.c` 里的旧校准残留、未再使用的 `extract_ypr()`；不碰 AI 阈值、协议和任务行为。
...
我先清掉 `stm32h7xx_it.c` 中已经失效的校准残留和解析函数，再删除 `Core/Inc/Backup`、`Core/Src/Backup` 两个确认过的备份目录。删除目录前会校验绝对路径仍在当前工作区内。
...
备份目录已经删除，`stm32h7xx_it.c` 的旧校准变量、注释块和未使用解析函数也清掉了。现在做静态检查，确认没有残留引用，也确认删除没有影响主线接口。
...
- 删除了备份目录：
  - `D:\Qian Start\Smart Watch\H743\Core\Inc\Backup`
  - `D:\Qian Start\Smart Watch\H743\Core\Src\Backup`
- 清理了 [stm32h7xx_it.c](<D:/Qian Start/Smart Watch/H743/Core/Src/stm32h7xx_it.c:77>) 里的旧校准残留：
  - `Calibrate_*`
  - `Calibrate_count`
  - `startRcv`
  - TIM2 中已注释掉的旧校准逻辑
  - 未再使用且乱码注释较多的 `extract_ypr()`
  - 不再需要的 `<stdlib.h>` include

验证结果：
- 静态搜索确认 `Calibrate_ / startRcv / extract_ypr / strtof / isdigit` 已无残留。
- `Core/Inc/Backup` 和 `Core/Src/Backup` 已不存在。
- 关键源文件 `{}` 数量平衡。
- `git diff --check` 通过，仅有 LF/CRLF 提示。
- 仍未跑 MDK 全量编译，因为当前 PATH 没找到 `UV4/armclang/armcc`。
