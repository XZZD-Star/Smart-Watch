  ...
  for(;;)
  {
    ...
    if (refresh_training_page != 0U)
    {
           ...
           refresh_pass++)
      {
        ...
        osDelay(SCREEN_PAGE_SETTLE_DELAY_MS);
        /* Rebuild after page switch so each forced refresh uses the latest counts. */
        screen_build_health_data(&screen_data, screen_time_base_tick);
...
void StartTask2(void *argument)
{
  ...
  for(;;)
  {
    ...
    for (;;)
    {
      ...
      {
        pending_test_value = g_test_conf_post_value;
        pending_test_confidence = g_test_conf_post_confidence;
        has_pending_test_conf = 1U;
      }
      if (g_action_kind_post_pending != 0U)
      {
        pending_action_kind_value = g_action_kind_post_value;
        has_pending_action_kind = 1U;
      }
      taskEXIT_CRITICAL();
      ...
      if ((subscribe_logged != 0U) && (has_pending_test_conf != 0U))
      {
        ...
        taskENTER_CRITICAL();
        if ((g_test_conf_post_pending != 0U) &&
            (g_test_conf_post_value == pending_test_value))
        {
      ...
      if ((subscribe_logged != 0U) && (has_pending_action_kind != 0U))
      {
        ...
        taskENTER_CRITICAL();
        if ((g_action_kind_post_pending != 0U) &&
            (g_action_kind_post_value == pending_action_kind_value))
        {
  ...
  const motion_ai_result_t *result)
{
  ...
  if ((result->test_done != 0U) && (g_task1_output_state.recognition_done_reported == 0U))
  {
    ...
    if (OneNet_TakeLastValidTestValue(&test_value) != 0U)
    {
      ...
      g_test_conf_post_confidence = result->top1_prob_avg;
...
/* Optional legacy probe path, compiled only when explicitly selected. */
#if APP_UART7_IS_SCREEN && APP_SCREEN_IS_COMPONENT_PROBE
static void screen_component_probe_init(void)
{
